...
환경변수를 이용하면 동일한 애플리케이션 이미지를 여러 컨테이너에서 실행하면서 각각 다른 서버 이름을 부여할 수 있습니다.

```mermaid
flowchart LR

  Image["simple-api<br/>Docker Image"]

  Image --> App1["api-01<br/>SERVER_NAME=api-01"]
  Image --> App2["api-02<br/>SERVER_NAME=api-02"]
  Image --> App3["api-03<br/>SERVER_NAME=api-03"]
```

애플리케이션 코드를 변경하지 않고 실행 환경만 다르게 설정할 수 있기 때문에 이후 로드밸런싱 실습에도 적합합니다.
