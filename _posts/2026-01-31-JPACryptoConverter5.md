...
`AttributeConverter`를 활용해서 애플리케이션 레벨 암호화의 개념을 간단하게 구현할 수 있었습니다. 하지만, `AttributeConverter`는 암호화 로직의 실행 시점과 자동화를 쉽게 해 줄 뿐입니다. 높은 수준의 보안성을 보장하는 것은 아니기 때문에, 실무에 적용 시에는 암호화 알고리즘/키 길이/IV 처리/안전한 키 저장 등에 대한 충분한 검토가 필요합니다.
