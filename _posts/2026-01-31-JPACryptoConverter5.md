...
이 방식은 실무에서도 자주 사용되는 패턴이지만,
다음과 같은 점은 반드시 고려해야 합니다.
...
`AttributeConverter`를 활용해서 애플리케이션 레벨 암호화의 기본 개념을 구현했습니다. 하지만 `AttributeConverter`는 암호화 로직의 실행 시점과 자동화를 쉽게 해 줄 뿐이지, 높은 수준의 보안성을 보장하는 것은 아닙니다. 

암호화 알고리즘/키 길이/IV 처리/안전한 키 저장 등은 여전히 신경써야 할 별도의 과제입니다.
