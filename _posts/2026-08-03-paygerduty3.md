...
### 2.2. Active Service EQ(현재 처리 중인 요청)

Active Service EQ 창에서는 현재 처리 중인 요청(Active Service)의 개수 변화를 실시간으로 확인할 수 있습니다.

<img class="main-image" src="/assets/images/active-service.jpg" alt="Active Service EQ">

현재는 모니터링 대상 애플리케이션으로 유입되는 요청이 없는 상태이기 때문에 처리 중인 요청의 개수도 0으로 표시됩니다. 하지만 많은 요청이 동시에 유입되거나, 일부 요청의 처리 시간이 길어지면 처리 중인 요청의 개수가 빠르게 증가하는 모습을 확인할 수 있습니다.

만약 이 수치가 지속적으로 높은 상태를 유지한다면, 애플리케이션이 요청을 제때 처리하지 못하고 있을 가능성이 있습니다. 이러한 경우에는 데이터베이스 조회 지연, 외부 API 호출 지연, 스레드 부족, 락 경합 등 요청 처리가 지연되는 원인이 없는지 함께 확인해 보는 것이 좋습니다.
