...
이전 포스팅에서 애플리케이션 레벨 암호화의 구현 방식을 고민했던 과정을 정리했습니다. 이제부터는 그 고민의 결과로 선택한 `AttributeConverter` 인터페이스를 통해, 실제로 애플리케이션 레벨 암호화가 어떻게 구현되었는지 코드 중심으로 살펴보도록 하겠습니다.

## 프로젝트 구조

먼저, 이번 토이 프로젝트의 전체 구조부터 살펴보겠습니다.
