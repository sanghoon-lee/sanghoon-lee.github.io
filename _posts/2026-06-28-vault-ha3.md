...
description: Vault 클러스터에서 Active 노드 장애를 재현하고 Leader Election과 Failover가 어떻게 동작하는지 단계별로 검증합니다. 또한 Request Forwarding 기능도 함께 살펴봅니다.
...
Active 노드에 장애가 발생하더라도 서비스가 중단 없이 계속 제공될 수 있어야 합니다. 이를 위해서는 장애 상황에서 Leader Election과 Failover가 정상적으로 동작해야 합니다.

그래서 이번 포스팅에서는 지난 포스팅에서 구축한 클러스터에 장애 상황을 재현하고, 어떻게 동작하는지 직접 확인해보도록 하겠습니다.
...
**참고**
> KV Engine을 활성화하고 시크릿 관리 기능을 사용하는 방법은 [[토이프로젝트] Vault를 이용한 KMS 대체 검토: (2) KV Engine을 이용한 암호화 키 저장과 조회 실습](https://sanghoon-lee.github.io/2026/04/10/vault1-1-rewriting/)에서 다룬 적이 있습니다. Vault 사용이 익숙하지 않은 경우 참고하시면 좋습니다.


Secret Engine의 목록을 조회해서 KV Engine이 명령에 인수로 지정했던 `secret/` 경로에 정상적으로 마운트되어 있는지 확인했습니다.
...
KV Engine이 마운트된 `secret/` 경로 하위에 `vault-test`를 추가하고, 두 쌍의 Key-Value 데이터를 저장했습니다. 
...
이는 KV Engine v2의 동작 방식 때문입니다. KV Engine v2는 내부 API에서 KV Engine이 마운트된 경로 뒤에 `data/`를 추가해 사용합니다. 현재 KV Engine은 `secret/` 경로에 마운트되어 있으므로, CLI에서 `secret/vault-test`를 입력하더라도 내부적으로는 `secret/data/vault-test`로 요청이 변환됩니다.
...
KV Engine을 활성화하거나 시크릿을 저장하지 않았지만, `vault-2`에서도 name과 blog가 포함된 시크릿이 조회되었습니다.
...
또한, `vault-3`에서도 동일하게 name과 blog가 포함된 시크릿을 조회할 수 있었습니다.

결과적으로 `vault-1`, `vault-2`, `vault-3`의 설정과 데이터가 동일하게 유지되고 있음이 확인되었습니다.
...
`vault-1` 노드를 종료하여 Active 노드에 장애가 발생한 상황을 재현했습니다.
...
출력 결과에서 종료된 `vault-1`은 follower로 표시되었고, `vault-2`가 새로운 leader로 선출된 것을 확인할 수 있었습니다.
...
지금까지는 컨테이너에 접속한 상태에서 Vault CLI를 통해 시크릿을 조회했습니다. 하지만 이번에는 각 노드의 장애 상태와 관계없이 외부 클라이언트의 관점에서 동작을 검증하기 위해 Vault에서 제공하는 REST API를 사용했습니다.
...
Active 노드에서 장애가 발생했지만, 새롭게 Active 노드로 선출된 `vault-2`에서는 시크릿을 정상적으로 조회할 수 있었습니다.
...
이유를 찾아보니 Vault의 Request Forwarding 기능 때문이었습니다.
...
Vault의 Raft 기반 클러스터에서는 Request Forwarding이 기본적으로 활성화됩니다. 이로 인해 클라이언트는 Standby 노드로 요청을 보내더라도 Active 노드가 처리한 정상적인 응답을 받을 수 있었습니다. 

하지만 클라이언트가 여전히 각 노드의 주소를 직접 알고 있어야 한다는 점은 그대로 남아 있습니다.

다음 포스팅에서는 Load Balancer를 배치해 Vault 클러스터의 단일 진입점을 구성해보겠습니다. 단일 진입점을 구성하면 클라이언트가 각 노드의 주소를 직접 지정하지 않고 Load Balancer의 주소만 사용해 요청을 보낼 수 있습니다.

---

[다음 포스팅으로 이동](https://sanghoon-lee.github.io/2026/07/04/vault-ha4/)
