...
따라서 서비스 초기부터 데이터가 어떤 형태로 저장되어야 하는지 기준을 정의하고 관리하는 것이 중요합니다.
...
동일한 전화번호이지만, 서로 다른 데이터로 시스템에서 인식되는 문제 발생하는 사례입니다.
...
동일한 코드이지만, 대소문자 차이로 인해 동일 코드 조회가 실패하는 사례입니다.

---

### 2.3 필수값 누락

회원 데이터 생성 시 필수 정보 누락으로 후속 서비스 이용에 오류가 발생하는 사례입니다.

---

## 3. 데이터 품질 관리를 위한 핵심 요소

데이터 품질을 유지하기 위한 핵심은 **정규화(normalize)**와 **검증(validate)**입니다. 정규화는 **데이터의 표현 방식을 통일하는 과정**이고, 검증은 **데이터가 정상적인 값인지 확인하는 과정**입니다. 즉, 정규화는 데이터의 형태를 관리하고, 검증은 데이터의 유효성을 관리합니다.
...
A[사용자 입력]
B[데이터 정규화<br/>Normalize]
C[데이터 검증<br/>Validate]
D[DB 저장]
...
먼저 입력된 데이터를 기준에 맞게 변환한 뒤 정상적인 데이터인지 검증해야 합니다.

만약 검증 이후에 정규화를 수행하면, 동일한 의미를 가진 데이터가 서로 다른 값으로 판단될 수 있습니다.
...
A[Client<br/>입력값 검증]
B[API Layer<br/>형식 검증]
C[Service Layer<br/>비즈니스 검증]
D[Database<br/>무결성 검증]
