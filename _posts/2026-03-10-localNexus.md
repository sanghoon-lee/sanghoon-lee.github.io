...
지금까지 `Nexus`를 설치해 `사설 라이브러리 저장소`를 구축하고, 라이브러리를 배포하는 과정을 살펴보았습니다.

소프트웨어 개발 조직의 `기술력`은 팀이 함께 활용할 수 있도록 조직에 축적된 **지식**과 **경험**의 크기와 깊이라고 생각합니다.

따라서 `사설 라이브러리 저장소`에 이러한 **지식**과 **경험**을 `코드` 형태의 라이브러리로 정리하는 것은 조직의 기술력을 실제로 활용할 수 있는 기반과 체계를 만들어가는 중요한 과정이라고 생각합니다.